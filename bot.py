...
async def cmd_spam_shortcut(message: Message):
    ...
    active_posts = len([f for f in os.listdir("spam_bot/texts") if f.endswith('.txt')]) if os.path.exists("spam_bot/texts") else 0
...
async def cb_spam_manage_channels(query: CallbackQuery):
    ...
    # Read current channels from config
    config_file = "spam_bot/config.json"
    channels = []
    if os.path.exists(config_file):
        with open(config_file, 'r', encoding='utf-8') as f:
            config = json.load(f)
            channels = config.get('groups', [])
...
async def cb_spam_do_delete_channel(query: CallbackQuery):
    ...
    if os.path.exists(config_file):
        ...
        await query.answer("❌ Не найден конфигурационный файл!", show_alert=True)
